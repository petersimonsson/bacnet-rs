...
use bacnet_rs::transport::{
    BacnetIpConfig, BacnetIpTransport, Transport, TransportError, TimeoutConfig,
    timeout_utils,
};
use std::{
    net::SocketAddr,
...
fn demo_timeout_configuration() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Registration timeout: {:?}", default_config.registration_timeout);
    println!("  Discovery timeout: {:?}", default_config.discovery_timeout);
    println!("  Property read timeout: {:?}", default_config.property_read_timeout);
    println!("  File transfer timeout: {:?}", default_config.file_transfer_timeout);
...
fn demo_socket_timeouts() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match transport.receive_timeout(Duration::from_millis(500)) {
        ...
        Err(TransportError::IoError(e)) if e.kind() == std::io::ErrorKind::WouldBlock => {
            println!("✓ Read timeout occurred (would block) after {:?}", start.elapsed());
        }
        Err(e) => {
        ...
        match transport.receive_timeout(timeout) {
            ...
            Err(_) => {
                ...
                if elapsed >= timeout && elapsed <= timeout + tolerance {
                    ...
                    println!("⚠ Timeout outside tolerance: expected ~{:?}, got {:?}", 
                        timeout, elapsed);
                }
            }
        }
...
fn demo_request_timeouts() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let invoke_id1 = transport.send_confirmed_request(
        target, 
        dummy_data, 
    ...
    let invoke_id2 = transport.send_confirmed_request(
        target, 
        dummy_data, 
    ...
    for (invoke_id, remaining) in requests_by_time {
        println!("  Request {}: {:.2}s remaining", invoke_id, remaining.as_secs_f64());
    }
    ...
    println!("Active requests after completion: {}", transport.active_request_count());
...
fn demo_adaptive_timeouts() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for safety_factor in safety_factors {
        ...
        println!("Safety factor {}: adaptive timeout = {:?}", 
    ...
    println!("Empty history: adaptive timeout = {:?} (should equal base)", adaptive_timeout);
    ...
    let consistent_adaptive = timeout_utils::calculate_adaptive_timeout(
        &consistent_times,
        base_timeout,
        2.0,
    );
    println!("Consistent times: adaptive timeout = {:?}", consistent_adaptive);
    ...
    let variable_adaptive = timeout_utils::calculate_adaptive_timeout(
        &variable_times,
        base_timeout,
        2.0,
    );
...
fn demo_retry_backoff() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Operation will fail {} times before succeeding", max_failures);
...
impl TimeoutStats {
    ...
    fn record_operation(&mut self, operation: &str, duration: Duration, timed_out: bool) {
        let stats = self.operations.entry(operation.to_string()).or_insert(OperationStats {
            total_count: 0,
            timeout_count: 0,
