...
use bacnet_rs::{
    ...
    network::Npdu,
    object::{Device, ObjectIdentifier, ObjectType, PropertyIdentifier, BacnetObject},
    service::{WhoIsRequest, IAmRequest, ReadPropertyRequest, UnconfirmedServiceChoice},
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Decoded Who-Is matches device: {}", decoded_whois.matches(12345));
    ...
    let read_prop = ReadPropertyRequest::new(
        device_id,
    ...
    println!("Encoded Read Property APDU: {} bytes", encoded_read_prop.len());
    ...
    println!("Decoded NPDU version: {}, consumed: {} bytes", decoded_npdu.version, consumed);
    println!("NPDU expecting reply: {}", decoded_npdu.control.expecting_reply);
