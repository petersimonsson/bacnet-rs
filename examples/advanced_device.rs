...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("\nCreated Objects:");
    println!("================");
    println!("Device: {} (Instance: {})", device.object_name, device.identifier.instance);
    ...
    println!("AO-1 Effective Priority: {:?}", ao1.get_effective_priority());
    ...
    println!("AO-1 Present Value: {:.1}% (back to priority 8)", ao1.present_value);
    ...
    println!("Created RPM request for {} objects", rpm_request.read_access_specifications.len());
    
    for (i, spec) in rpm_request.read_access_specifications.iter().enumerate() {
        println!("  Object {}: {:?} with {} properties", 
                 i + 1, 
    ...
    if !ai_subs.is_empty() {
        println!("  AI-1 subscription time remaining: {} seconds", ai_subs[0].time_remaining);
    }
    
    let ao_subs = cov_manager.get_subscriptions_for_object(ao_id);
    if !ao_subs.is_empty() {
        println!("  AO-1 subscription time remaining: {} seconds", ao_subs[0].time_remaining);
    }
    ...
    println!("Priority level: {} (0=Life Safety, 1=Critical, 2=Urgent, 3=Normal)", 
    ...
    for property in &[PropertyIdentifier::ObjectName, PropertyIdentifier::PresentValue, PropertyIdentifier::OutOfService] {
        println!("  AI-1 {:?}: {}", property, ai1.is_property_writable(*property));
        println!("  AO-1 {:?}: {}", property, ao1.is_property_writable(*property));
    }
