...
impl NetworkAddress {
    ...
    pub fn new(network: u16, address: Option<Vec<u8>>) -> Self {
    ...
    pub fn decode(data: &[u8]) -> Result<(Self, usize)> {
        ...
        Ok((Self::new(network, address), pos))
    }
...
impl Npdu {
    ...
    pub fn decode(data: &[u8]) -> Result<(Self, usize)> {
        ...
        let destination = if control.destination_present {
            let (address, offset) = NetworkAddress::decode(&data[pos..])?;
            pos += offset;
            Some(address)
        } else {
            None
...
mod tests {
    ...
    fn test_npdu_with_destination() {
        ...
        npdu.destination = Some(NetworkAddress::new(100, Some(vec![192, 168, 1, 1])));
    ...
    fn test_routing_table() {
        ...
        let router = RouterInfo {
            ...
            address: NetworkAddress::new(0, Some(vec![192, 168, 1, 1])),
    ...
    fn test_router_manager() {
        ...
        manager.add_discovered_router(
            vec![100],
            NetworkAddress::new(0, Some(vec![192, 168, 1, 1])),
            Some(10),
        );

        // Test routing a message to network 100
        let mut npdu = Npdu::new();
        npdu.destination = Some(NetworkAddress::new(100, Some(vec![10, 0, 0, 1])));
        npdu.hop_count = Some(5);
        ...
        local_npdu.destination = Some(NetworkAddress::new(1, Some(vec![10, 0, 0, 1])));
        ...
        hopless_npdu.destination = Some(NetworkAddress::new(100, Some(vec![10, 0, 0, 1])));
        ...
        unreachable_npdu.destination = Some(NetworkAddress::new(999, Some(vec![10, 0, 0, 1])));
    ...
    fn test_path_discovery() {
        ...
        discovery.add_link(NetworkLink {
            ...
            cost: 10,
            router_address: NetworkAddress::new(0, Some(vec![192, 168, 1, 1])),
        });
        ...
        discovery.add_link(NetworkLink {
            ...
            cost: 15,
            router_address: NetworkAddress::new(0, Some(vec![192, 168, 2, 1])),
        });
    ...
    fn test_router_health() {
        let mut diagnostics = NetworkDiagnostics::new();
        let router_addr = NetworkAddress::new(0, Some(vec![192, 168, 1, 1]));
    ...
    fn test_network_address_properties() {
        let local_addr = NetworkAddress::new(0, Some(vec![192, 168, 1, 1]));
        assert!(local_addr.is_local());
        assert!(!local_addr.is_broadcast());

        let broadcast_addr = NetworkAddress::new(0xFFFF, None);
        assert!(broadcast_addr.is_broadcast());
        assert!(!broadcast_addr.is_local());

        let remote_addr = NetworkAddress::new(100, Some(vec![10, 0, 0, 1]));
